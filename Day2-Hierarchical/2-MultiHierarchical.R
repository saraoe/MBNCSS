...
group_des <- group_design(
     list(
          v_Sd ~ age, `v_Sd:log_freq` ~ age, a ~ age,
          t0 ~ age
     ),
     data = data_full,
...
# Update our prior
prior_age <- prior(alt_design,
     type = "standard", group_design = group_des,
     update = prior_alt
)
prior_age
DDM_age <- make_emc(
     data = data_full, design = alt_design, prior_list = prior_age,
...
DDM_uni <- make_emc(
     data = data_full, design = alt_design, prior_list = prior_uni,
     group_design = group_des
)
